...
def get_login_params():
    ...
    params = {
        ...
        'js_type': 1,
        'login_sig': '',
        'u1': 'http%3A%2F%2Flixian.qq.com%2Fmain.html',
        'r': 0.9659168085697463,
        'pt_uistyle': 40
    }
    headers = {
    ...
    textview = re.findall("'(.*?)'", r.text)
    verifycode, pt_verifysession_v1 = textview[1], textview[3]
    cookies = r.cookies
    return {
...
def login(data):
    ...
    pt_verifysession_v1 = data['pt_verifysession_v1']
    cookies = data['cookies']
    p = ctx.call("getEncryption", password, user, verifycode, "")

    url = 'http://ptlogin2.qq.com/login?u=' + user + '&verifycode=' + verifycode + '&pt_vcode_v1=0&pt_verifysession_v1=' + pt_verifysession_v1 + '&p='+ p + \
            '&pt_randsalt=0&u1=http%3A%2F%2Flixian.qq.com%2Fmain.html&ptredirect=1&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=3-25-1480238766570&js_ver=10181' \
            '&js_type=1&login_sig=2HBX-OBx7uWiwl8nzAdemebYOwk8GofFVD0p9D9QkOKAGZioRL9Tj5QPYoRAdd9F&pt_uistyle=40&aid=567008010&'
    r = requests.get(url=url, cookies=cookies)
    if "登录成功" in r.text:
        textview = re.findall("'(.*?)'", r.text)
        cookies = r.cookies
        print('用户 "%s" : %s' % (textview[5], textview[4]))
    elif "参数错误" in r.text:
        raise ValueError('该账号登录需要验证码！请稍后重试（验证码验证消失）')
    return cookies


def get_parse_torrent_cookies(cookies):
    """获取解析种子所需cookie"""
    skey = cookies['skey']
    g_tk = ctx_g_tk.call("getACSRFToken", skey)

    url = 'http://lixian.qq.com/handler/lixian/do_lixian_login.php'
    data = {
        'g_tk': g_tk
    }
    # header
    ...
    newcookies = cookies.copy()
    newcookies.update(r.cookies)
    return newcookies

def parse_torrent(cookies):
    torrentname = input("请输入要解析的种子文件名称（请确保文件路径和程序在同一目录）: ")

    url = 'http://lixian.qq.com/handler/bt_handler.php?cmd=readinfo'
    files = {
        'myfile': (torrentname, open(torrentname, 'rb'), 'application/octet-stream')  # 提交的种子文件
    }
    headers = {
        'Referer': 'http://lixian.qq.com/main.html'
    }
    ...
    data = simple_screening(r=r)
    # data = get_mid(data=data, cookies=cookies)
    # get_file_hash(cookies)


def simple_screening(r):
    """简单的删除非视频文件"""
    data_json = json.loads(r.text)
    torrent_hash = data_json['hash']
    taskname = data_json['name']
    file_sizeALL = data_json['files']
    size_index_filename = ()  # 保存有效的文件大小和索引以及文件名
    for everysize in file_sizeALL:
        current_size = "23" + str(everysize['file_size_ori'])
        current_index = everysize['file_index']
        current_filename = everysize['file_name']
        size_index_filename = (str(everysize['file_size_ori']), current_index, current_filename)
        if int(current_size) >= 231024000:  # 粗略的估计，去除非视频文件
            break;
    filesize, index, filename = size_index_filename
    print("解析的种子电影名：%s (size： %.2fG)" % (filename, (int(filesize) / 1024 ** 3)))
    print('种子磁力：magnet:?xt=urn:btih:%s' % torrent_hash)
    return {
...
def get_mid(data, cookies):
    ...
    url = 'http://lixian.qq.com/handler/xfjson2012.php'
    headers = {
        'Referer': 'http://lixian.qq.com/main.html'
    }
    data = {
...
def get_file_hash(cookies):
    """得到file_hash--腾讯服务器存在问题无法解析"""
    mid = '1524410605074559198%2C1524410605124561198%2C1524410605064213198'
    url = 'http://lixian.qq.com/handler/lixian/get_lixian_status.php'
    headers = {
        'Referer': 'http://lixian.qq.com/main.html'
    }
    data = {
...
if __name__ == '__main__':
    cookies = login(get_login_params())
    parse_torrent(get_parse_torrent_cookies(cookies=cookies))
